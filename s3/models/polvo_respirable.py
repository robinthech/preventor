...
class ProductProduct(models.Model):
	...
	polvo_respirable_ids = fields.One2many('polvo.respirable', 'registro_id', string=u'Polvo Respirable')
	puntos_respi = fields.Integer(compute="_compute_puntos_respi", string=u"Polvo Respirable")

	@api.depends("polvo_respirable_ids")
	def _compute_puntos_respi(self):
		for record in self:
			record.puntos_respi = len(record.polvo_respirable_ids)


class PolvoRespirable(models.Model):
	_name = "polvo.respirable"
	...
	@api.depends("codigo", "hora_inicio", "hora_fin", "flujo_promedio", "tiempo_expo", "jornada", "tiempo_medio", "volumen", "peso_inicial", "peso_final",
				 "muestra", "concentracion", "concentracion_real", "tlv", "indice", "cumple")
	def _compute_polvo(self):
		for record in self:
			...
			if record.flujo_promedio:
				record.volumen = round((record.tiempo_medio*record.flujo_promedio)/1000, 2)
			else:
				record.volumen = 0.00
			...
			if record.jornada != 0.00:
				if record.tiempo_expo:
					...
					record.tlv = ((8/record.jornada)*((24-record.jornada)/16))*3
				else:
